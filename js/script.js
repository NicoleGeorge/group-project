$(".btnStyle").on("click", function () {
  ...
  // Character Info API
  var queryURL =
    "https://www.anapioficeandfire.com/api/characters/?name=" + search;
  $.ajax({
    url: queryURL,
    method: "GET",
  }).then(function (data) {
    ...
    var faveCharacter = $("<p class='results-title'>").text(
      "Meet your favourite GoT character, "
    );
    ...
    var titles = $("<p>").text(
      "Their official title is, " + data[0].titles + "."
    );
    $(".content1").append(titles);
    var aliases = $("<p>").text(
      "But they are also known as, " + data[0].aliases
    );
  ...
  // Giphy API
  var queryGiphy =
    "https://api.giphy.com/v1/gifs/search?api_key=w5F8XRxKHMrbEYBn1oSBE3J3gmsYZ9v2&limit=1&q=";
